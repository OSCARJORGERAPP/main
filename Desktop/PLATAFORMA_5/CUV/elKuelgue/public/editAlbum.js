document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (albumId) {
        try {
            const response = await axios.get(`https://elKuelgue.onrenders.com:port/albums/${albumId}`);
    ...
    document.getElementById('albumForm').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await axios.put(`https://elKuelgue.onrenders.com:port/albums/${albumId}`, {
                titulo,
                fechaDeLanzamiento,
                descripcion,
                portada
    ...
    document.getElementById('cancionesForm').addEventListener('submit', async function(event) {
        ...
        try {
            const response = await axios.post(`https://elKuelgue.onrenders.com:port/albums/${albumId}/canciones`, {
                nombreDeCancion,
                duracion,
                link
            });
